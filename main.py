...
def write_to_file(x, y, dy, ddy, count, filename, mode):
    ...
    for i in range(count):
        if i > 0:
            ...
            f1.write(", ")
            f2.write(", ")
        f.write('{' + str(x[i]) + ', ' + str('{:f}'.format(y[i])) + '}')
        f1.write('{' + str(x[i]) + ', ' + str('{:f}'.format(dy[i])) + '}')
        f2.write('{' + str(x[i]) + ', ' + str('{:f}'.format(ddy[i])) + '}')
    f.write('}\n')
    f1.write('}\n')
    f2.write('}\n')
...
def K3_f1_3(x, u1, u2):
    return f1(x + h, u1 - h * K1_f1_3(x, u1, u2) + 2 * h * K2_f1_3(x, u1, u2), u2 - h * K1_f2_3(x, u1, u2) + 2 * h * K2_f2_3(x, u1, u2))

def K3_f2_3(x, u1, u2):
    return f2(x + h, u1 - h * K1_f1_3(x, u1, u2) + 2 * h * K2_f1_3(x, u1, u2), u2 - h * K1_f2_3(x, u1, u2) + 2 * h * K2_f2_3(x, u1, u2))
...
if __name__ == "__main__":
    order = input('Порядок метода?\n> ')
    order = int(order)
    mode = input('Вывести для сходимости (4 раза уменьшаем шаг) или для одного шага? (1 or 2)\n> ')
    mode = int(mode)
    if mode == 1:
